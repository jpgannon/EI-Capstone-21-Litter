    ...
    dashboardBody(tabItems(
        ...
        tabItem(tabName = "Litterfall_BiVar", 
                ...
                column(6, h4("Litterfall Plot Explorer"),
                       sliderInput("Year_Bivar", "Year Range:",
        ...
        tabItem(tabName = "SoilRespiration_BiVar", 
                ...
                column(2, h4("Soil Respiration Plot Explorer"),
                       sliderInput("date_bivariate", "Date:",
                       ...
                       br(),
                       checkboxInput('jittersoil', 'Jitter'),
                       checkboxInput('smoothsoil', 'Smooth')
                ),
                column(3, offset = 1,
                       selectInput('xsoil', 'X-Axis:', names(dataset1)),
                       selectInput('ysoil', 'Y-Axis:', names(dataset1), names(dataset1)[[2]]),
                       selectInput('colorsoil', 'Color by', c('None', names(dataset1)))
...
server <- function(input, output) {
    ...
    dataset <- reactive({
        dataset <- as.data.frame(Litterfall[sample(nrow(Litterfall), input$Year_Bivar),])
    })
    ...
    dataset1 <- reactive({
       dataset1 <- as.data.frame(CleanSoilResp[sample(nrow(CleanSoilResp), input$date_bivariate),])
    })
    
    output$plot1 <- renderPlot({
        
        p <- ggplot(dataset1(), aes_string(x=input$xsoil, y=input$ysoil)) + geom_point()
        
        if (input$colorsoil != 'None')
            p <- p + aes_string(color=input$colorsoil)
