  ...
  dashboardBody(tabItems(
    ...
    tabItem(tabName = "Litterfall_BiVar", 
            ...
            fluidRow(column(4, offset = 1,
                   selectInput('x', 'X', names(dataset)[[2]]),
                   selectInput('y', 'Y', names(dataset), names(dataset)[[7]]),
                   selectInput('color', 'Color', c('None', names(dataset))[[4]]))
    )),
    ...
    tabItem(tabName = "SoilRespiration_BiVar", 
            ...
            hr(),
            column(4, h4("Soil Respiration Plot Explorer"),
            ...
            fluidRow(column(3, offset = 1,
                   selectInput('x1', 'X', names(dataset1)[[5]]),
                   selectInput('y1', 'Y', names(dataset1), names(dataset1)[[4]]),
                   selectInput('color1', 'Color', c('None', names(dataset1))[[4]])
...
server <- function(input, output) {
  ...
  dataset1 <- reactive({
    GroupedSoilData[sample(nrow(GroupedSoilData), ),]
  })
